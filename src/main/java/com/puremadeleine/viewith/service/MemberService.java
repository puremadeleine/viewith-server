...
public class MemberService extends SpringProxyAware<MemberService> {
    ...
    public JoinResDto login(OAuthType authType, String token) {
        return switch (authType) {
            case KAKAO -> getProxy().loginByKakao(token);
            case APPLE -> loginByApple(token);
            default -> throw new ViewithException(ViewithErrorCode.INVALID_PARAM);
        };
    }

    @Transactional
    public JoinResDto loginByKakao(String oauthAccessToken) {
        // Kakao 인증 및 유저 정보 조회
//        AccessTokenResDto tokenInfo = kakaoService.getAccessToken(code);
//        String oauthAccessToken = tokenInfo.getAccessToken();
        AccessTokenInfoResDto tokenInfo = kakaoService.getAccessTokenInfo(oauthAccessToken);
        ...
        MemberEntity member = optionalMember.orElseGet(() -> createAndSaveNewMember(kakaoUserInfo));
        ...
        String refreshToken = jwtService.makeRefreshToken(memberInfo, tokenInfo.getExpiresIn()); //tokenInfo.getRefreshTokenExpiresIn()
